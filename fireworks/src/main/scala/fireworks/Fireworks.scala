...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then 
      copy(position = Motion.movePoint(point = position, direction = direction, speed = Settings.propulsionSpeed),
      countDown = countDown - 1)
    else
      Exploding.init(numberOfParticles = numberOfParticles, direction = direction, position = position, color = particlesColor)
...
case class Particle(horizontalSpeed: Double, verticalSpeed: Double, position: Point, color: Color):
  ...
  def next: Particle =
    ...
    val updatedPosition = 
      Point(position.x + updatedHorizontalSpeed, position.y + updatedVerticalSpeed)
    // Construct a new particle with the updated position and speed
    Particle(horizontalSpeed = updatedHorizontalSpeed, verticalSpeed = updatedVerticalSpeed, position = updatedPosition, color = color)
