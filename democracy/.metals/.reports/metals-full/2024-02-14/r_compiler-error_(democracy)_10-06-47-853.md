file://<WORKSPACE>/src/main/scala/democracy/MajorityJudgement.scala
...
Scala version: 3.3.0
Classpath:
<WORKSPACE>/.bloop/democracy/bloop-bsp-clients-classes/classes-Metals-inAfShllRWmkvqkhV5ZsrA== [exists ], <HOME>/.cache/bloop/semanticdb/com.sourcegraph.semanticdb-javac.0.9.9/semanticdb-javac-0.9.9.jar [exists ], <HOME>/.cache/coursier/v1/https/repo1.maven.org/maven2/org/scala-lang/scala3-library_3/3.3.0/scala3-library_3-3.3.0.jar [exists ], <HOME>/.cache/coursier/v1/https/repo1.maven.org/maven2/org/scala-lang/scala-library/2.13.10/scala-library-2.13.10.jar [exists ]
Options:
...
action parameters:
offset: 4778
uri: file://<WORKSPACE>/src/main/scala/democracy/MajorityJudgement.scala
text:
```scala
package democracy
...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
    val new_seq = grades.sortBy(_.ordinal)
    val size = new_seq.size
    if size %2 == 0 then
        new_seq.apply((size/2)+1)
    else
        new_seq.apply((size+1)/2)
end Grade
  ...
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      val bestMedianGrade: Grade =
        gradesPerCandidate.values          // Sélectionnez les collections de grades
